...
function BluePhysicalSpell(caster, target, spell, params)
    ...
    local D =  math.floor((magicskill * 0.15)) * 2 + 3;
    -- cap D
    ...
    local atkratio = params.offcratiomod / target:getStat(dsp.mod.DEF);
    local isSneakValid = caster:hasStatusEffect(dsp.effect.SNEAK_ATTACK)
    if (isSneakValid and not (caster:isBehind(target) or caster:hasStatusEffect(dsp.effect.HIDE) or target:hasStatusEffect(dsp.effect.DOUBT))) then
        isSneakValid = false
    end
    caster:delStatusEffectsByFlag(dsp.effectFlag.DETECTABLE)
    caster:delStatusEffect(dsp.effect.SNEAK_ATTACK)
    if(isSneakValid) then
        atkratio = atkratio + 1;
    end
    print(params.offcratiomod)
    local cratio = BluecRatio(atkratio, caster:getMainLvl(), target:getMainLvl(), isSneakValid);
    local hitrate = 0.05;
    if(caster:hasStatusEffect(dsp.effect.TRICK_ATTACK)) then
        hitrate = 1.0;
        caster:delStatusEffect(dsp.effect.TRICK_ATTACK)
    else
        hitrate = BlueGetHitRate(caster,target,true);
    end
...
function BlueGetWsc(attacker, params)
    wsc = attacker:getStat(dsp.mod.STR) * params.str_wsc + attacker:getStat(dsp.mod.DEX) * params.dex_wsc +
         ...
         attacker:getStat(dsp.mod.CHR) * params.chr_wsc;
    return wsc;
end;

-- Given the raw ratio value (atk/def) and levels, returns the cRatio (min then max)
function BluecRatio(ratio,atk_lvl,def_lvl, isCrit)
    -- Level penalty...
    local levelcor = 0;
    if (atk_lvl < def_lvl) then
        levelcor = 0.02 * (def_lvl - atk_lvl);
    end
    ...
    elseif ratio < 2.44 then
        cratiomin = ratio * 1176 / 1024 - 775 / 1024
    else
