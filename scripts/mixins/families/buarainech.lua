...
g_mixins.families.colibri_mimic = function(mob)
    ...
    mob:addListener("MAGIC_TAKE", "COLIBRI_MIMIC_MAGIC_TAKE", function(caster, target, spell)
        ...
        then
            target:setLocalVar("[colibri]spellToMimic", 196) -- which spell to mimic
    ...
    mob:addListener("COMBAT_TICK", "COLIBRI_MIMIC_CTICK", function(mob)
        ...
        if mob:AnimationSub() == 1 then
            if spellToMimic > 0 and osTime > castTime and castWindow > osTime then
                mob:addMod(dsp.mod.FASTCAST, 99);
                ...
                mob:AnimationSub(0)
                mob:delMod(dsp.mod.FASTCAST, 99);
