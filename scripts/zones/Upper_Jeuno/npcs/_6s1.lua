...
function onTrade(player,npc,trade)
    if(trade:getItemCount() == 1 and (trade:hasItemQty(ring[1],1) or trade:hasItemQty(ring[2],1) or trade:hasItemQty(ring[3],1))) then
        if(not player:hasItem(ring[1])) then
            player:addItem(ring[1])
            player:messageSpecial(ID.text.ITEM_OBTAINED, ring[1]);
        end
        if(not player:hasItem(ring[2])) then
            player:addItem(ring[2])
            player:messageSpecial(ID.text.ITEM_OBTAINED, ring[2]);
        end
        if(not player:hasItem(ring[3])) then
            player:addItem(ring[3])
            player:messageSpecial(ID.text.ITEM_OBTAINED, ring[3]);
        end
    end
end;
