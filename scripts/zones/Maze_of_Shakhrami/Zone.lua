...
function onInitialize(zone)
    if math.random(4) == 1 then
        ...
        GetMobByID(ID.mob.LEECH_KING):setRespawnTime(math.random(300, 3600)) -- math.random(900, 43200)
    else
        DisallowRespawn(ID.mob.LEECH_KING, true)
        DisallowRespawn(ID.mob.ARGUS, false)
        UpdateNMSpawnPoint(ID.mob.ARGUS)
        GetMobByID(ID.mob.ARGUS):setRespawnTime(math.random(300, 3600)) -- math.random(900, 43200)
    end
