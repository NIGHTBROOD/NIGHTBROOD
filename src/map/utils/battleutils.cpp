...
namespace battleutils
{
    ...
    float GetRangedDamageRatio(CBattleEntity* PAttacker, CBattleEntity* PDefender, bool isCritical)
    {
        ...
        if (PAttacker->objtype == TYPE_PC)
        {
            ...
            else
            {
                ...
                {
                    rAttack = PChar->RATT(PItem->getSkillType(), PItem->getILvlSkill());
                    if (PItem->getSlotType() == SKILL_MARKSMANSHIP)
                    {
                        ratiocap = 3.5f;
                    }
                    else
    ...
    ************************************************************************/
    uint8 GetBlockRate(CBattleEntity* PAttacker, CBattleEntity* PDefender)
    {
        ...
        uint16 baseblock = battleutils::GetMaxSkill(SKILL_SHIELD, PDefender->GetMJob(), PDefender->GetMLevel());
        ...
        {
            float skillmodifier = (PDefender->GetSkill(SKILL_AUTOMATON_MELEE) - battleutils::GetMaxSkill(SKILL_AUTOMATON_MELEE, PDefender->GetMJob(), PDefender->GetMLevel())) * 0.5f;
        ...
        if (PDefender->objtype == TYPE_PC)
        {
            lCorr = PDefender->GetMLevel() < PAttacker->GetMLevel() ? (PDefender->GetMLevel() - PAttacker->GetMLevel()) / 2 : 0;
        }
        float skillmodifier = (blockskill - baseblock) * 0.5f - lCorr;
        return (int8)std::clamp((int32)((base + (int32)skillmodifier) * blockRateMod), 5, (shieldSize == 6 ? 100 : std::max<int32>((int32)(80 * blockRateMod), 99)));
    }
    ...
    ************************************************************************/
    ...
    float GetDamageRatio(CBattleEntity* PAttacker, CBattleEntity* PDefender, bool isCritical, float bonusAttPercent)
    {
        ...
        {
            CCharEntity* PChar = (CCharEntity*)PAttacker;
            CItemWeapon* PItem = (CItemWeapon*)PChar->getEquip(SLOT_MAIN);
            if (PItem == nullptr || !PItem->isType(ITEM_WEAPON))
            {
                ratioCap = 3.25f;
            }
...
#define PAIR(x, y) ((x << 8) + y)
    ...
    ************************************************************************/
    ...
    uint32 CalculateSpellCastTime(CBattleEntity* PEntity, CMagicState* PMagicState)
    {
        ...
        {
            uint16 spellcasting = battleutils::GetMeritValue(PEntity, MERIT_SUMMONING_MAGIC_CAST_TIME);
            cast = (uint32)(cast * (1.0f - ((spellcasting > 25 ? 25 : spellcasting) / 100.0f)));
        }
